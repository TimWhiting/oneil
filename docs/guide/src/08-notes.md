...
If you omit `{…}`, the viewer derives a symbol from the identifier (`omega` → $\omega$, `A_hab` → $A_{hab}$, and so on). That same symbol is what `{{identifier:equation}}` uses when interpolating into notes. Full declaration shape is in [Parameters](./03-parameters.md).
...
Images use `![alt](./path.png)` (workspace root first, then relative to the model file). PDF paths in image links are recognized by the extension’s path diagnostics.
...
```oneil
    ~ O'Neill described Island Three in *The High Frontier* [@ONeill1977].
...
Prefer a file named **`references.bib`**. The extension searches, most preferred first:
...
### Citing a PDF (recommended setup)

Goal: click `[@key]` (or `[@key, p. N]`) in the Rendered View and open the right PDF on the right page.

**1. Put a remote source on the BibTeX entry**

At minimum, give the entry a `url` (direct PDF link preferred) and/or a `doi`:
...
1. BibTeX `file` (absolute, `~/…`, `./…` relative to workspace then model dir, or bare name in the PDF cache)
2. Cache entry already downloaded for that URL
3. Download from `url` / DOI (unless offline), then optionally update `references.bib`
4. Open the remote URL in the browser
...
| `url`, `doi` | Remote PDF / link; used to download into the cache |
